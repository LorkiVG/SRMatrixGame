...
DTRACE();
    ...
    if(light != 0) CMatrixEffect::CreatePointLight(&m_Light, pos, 1, light, false);
...
DTRACE();
    ...
    if(m_Handler) m_Handler(TRACE_STOP_NONE, D3DXVECTOR3(m_Mat._41,m_Mat._42,WATER_LEVEL), (dword)m_User, FEHF_LASTHIT);
}
...
DTRACE();
    ...
    if(m_TTL < 0)
    {
        if(m_Handler)
        {
            ...
            {
                if((m_Mat._43 - m_Radius) <= WATER_LEVEL) m_Handler(TRACE_STOP_WATER, D3DXVECTOR3(m_Mat._41, m_Mat._42, WATER_LEVEL), (dword)m_User, 0);
            }

            if(m_hitmask & TRACE_LANDSCAPE)
            {
                float z = g_MatrixMap->GetZ(m_Mat._41, m_Mat._42);
                if((m_Mat._43 - m_Radius) <= z) m_Handler(TRACE_STOP_LANDSCAPE, D3DXVECTOR3(m_Mat._41, m_Mat._42, z), (dword)m_User, 0);
            }

            if(m_hitmask & TRACE_ANYOBJECT) g_MatrixMap->FindObjects(*(D3DXVECTOR3*)&m_Mat._41, m_Radius, 1, m_hitmask, m_skip, BoomEnum, (dword)this);
        }
#ifdef _DEBUG
    ...
    if(m_Handler)
    {
        if(m_hitmask & TRACE_ANYOBJECT) g_MatrixMap->FindObjects(*(D3DXVECTOR3*)&m_Mat._41, m_Radius, 1, m_hitmask, m_skip, BoomEnumNaklon, (dword)this);
    }
    ...
    {
        if(k < 0.5f) g_MatrixMap->m_Camera.Stat();
    }
    ...
    if(m_Light.effect) ((CMatrixEffectPointLight *)m_Light.effect)->SetRadius(scale);
}
