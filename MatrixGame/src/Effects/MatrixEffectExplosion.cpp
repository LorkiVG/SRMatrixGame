...
DTRACE();
    ...
    for(; sparks_cnt > 0; ++i)
    {
        ...
        m_Deb[i].bline = (CSpriteLine*)HAlloc(sizeof(CSpriteLine), m_Heap);
        m_Deb[i].bline->CSpriteLine::CSpriteLine(TRACE_PARAM_CALL pos, pos + m_Deb[i].v * (1.0f / 20.0f), EXPLOSION_SPARK_WIDTH, 0xFFFFFFFF, GetSingleBrightSpriteTex(SPR_SPARK));
        m_Deb[i].ttl = (float)RND(m_Props->deb_min_ttl, m_Props->deb_max_ttl);
