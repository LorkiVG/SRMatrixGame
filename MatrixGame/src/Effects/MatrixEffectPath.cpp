...
void CMatrixEffectPath::Tact(float step)
{
DTRACE();
    ...
    m_Tact += step;
    if(m_Tact > m_NextTact || m_DotsCnt < m_DotsMax)
    {
        m_NextTact += PATH_DOT_RATE;

        while(m_DotsCnt < m_DotsMax)
        {
            LIST_INSERT(m_First, (m_Dots + m_DotsCnt), m_First, m_Last, prev, next);
            m_Dots[m_DotsCnt].pos = 0;
            if(m_SpriteTextures[SPR_PATH_PART].IsSingleBrightTexture()) m_Dots[m_DotsCnt].dot.CSprite::CSprite(TRACE_PARAM_CALL *(D3DXVECTOR3*)m_Points.Get(), PATH_SIZE, 0, PATH_COLOR, m_SpriteTextures[SPR_PATH_PART].tex);
            else m_Dots[m_DotsCnt].dot.CSprite::CSprite(TRACE_PARAM_CALL *(D3DXVECTOR3 *)m_Points.Get(), PATH_SIZE, 0, PATH_COLOR, &m_SpriteTextures[SPR_PATH_PART].spr_tex);
            ++m_DotsCnt;
        }
    }
