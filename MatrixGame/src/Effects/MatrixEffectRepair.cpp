...
CMatrixEffectRepair::CMatrixEffectRepair(const D3DXVECTOR3& pos, const D3DXVECTOR3& dir, float seekradius, CMatrixMapStatic* skip, ESpriteTextureSort sprite_spot) :
    CMatrixEffect(), m_SeekRadius(seekradius), m_Skip(skip), m_Pos(pos), m_Dir(dir), m_KordOnTarget(m_Heap), m_Kord(m_Heap), m_SpriteSpot(sprite_spot)
{
DTRACE();
...
#ifdef _DEBUG
    for(i = m_BBCnt; i < REPAIR_BB_CNT; ++i) m_BBoards[i].bb.release_called = true;
#endif
