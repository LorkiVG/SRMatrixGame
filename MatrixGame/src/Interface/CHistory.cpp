...
CHistory::CHistory()
{
    m_FirstConfig = nullptr;
    m_LastConfig = nullptr;
...
void CHistory::DelConfig(SRobotConfig *config)
{
    ...
    {
        bool gotcha = false;
        if(confs->m_Head.m_nKind == config->m_Head.m_nKind && confs->m_Chassis.m_nKind == config->m_Chassis.m_nKind && confs->m_Hull.m_Module.m_nKind == config->m_Hull.m_Module.m_nKind)
        {
            gotcha = true;       
        }
...
DTRACE();
    ...
    if(m_CurrentConfig)
    {
        ...
        ps->m_Constructor->ReplaceRobotModule(MRT_HEAD, m_CurrentConfig->m_Head.m_nKind, 0, 0, true);
        int cur_hull_num = m_CurrentConfig->m_Hull.m_Module.m_nKind;
        ps->m_Constructor->ReplaceRobotModule(MRT_HULL, (ERobotModuleKind)cur_hull_num, 0, 0, true);
        for(int i = 0; i < RUK_WEAPON_PYLONS_COUNT; ++i)
        {
            if(m_CurrentConfig->m_Weapon[i].m_nKind)
            {
                //ќпредел€ем фактический номер пилона на модели
                int model_pylon = 0;
                for(int j = 0; j < g_Config.m_RobotHullsConsts[cur_hull_num].weapon_pylon_data.size(); ++j)
                {
                    if(g_Config.m_RobotHullsConsts[cur_hull_num].weapon_pylon_data[j].constructor_slot_num == i)
                    {
                        model_pylon = j;
                        break;
                    }
                ...
                ps->m_Constructor->ReplaceRobotModule(MRT_WEAPON, m_CurrentConfig->m_Weapon[i].m_nKind, i, model_pylon, true);
            }
        }

        ps->m_Constructor->ReplaceRobotModule(MRT_CHASSIS, m_CurrentConfig->m_Chassis.m_nKind, 0, 0, true);
    }
}
