...
void CMatrixRoad::DeleteZone()
{
    ...
    if(m_Zone!=nullptr) { HFree(m_Zone,m_Parent->m_Heap); m_Zone=nullptr; }
    m_ZoneCnt=0;
    m_ZoneCntMax=0;
}
...
if(test && !g_TestLocal) {
D3DXVECTOR3 v2;
v2.x=rr->m_Modules[ln*m_CrotchCnt+rr->m_Header[ln].m_Cnt-1].m_Crotch->m_Center.x*GLOBAL_SCALE_MOVE;
v2.y=rr->m_Modules[ln*m_CrotchCnt+rr->m_Header[ln].m_Cnt-1].m_Crotch->m_Center.y*GLOBAL_SCALE_MOVE;
...
void CMatrixRoadNetwork::ClearPlace()
{
    m_PlaceCnt=0;
    m_PlaceEmpty=-1;
    if(m_Place!=nullptr) { HFree(m_Place,m_Heap); m_Place=nullptr; }
}
...
{
    if(m_Region!=nullptr) { HFree(m_Region,m_Heap); m_Region=nullptr; }
    m_RegionCnt=0;
}
