...
DCP();
    ...
    {
DCP();
        GetPlayerSide()->GetArcadedObject()->StaticTact(step);
        if(GetPlayerSide()->GetArcadedObject())
        {
            GetPlayerSide()->GetArcadedObject()->GetResources(MR_Matrix);
            //Используется для последующего выравнивания положения корпуса в ручном режиме управления
            GetPlayerSide()->InterpolateArcadedRobotHullPos(step);
        }
    }
DCP();
...
DTRACE();
    ...
    while(obj)
    {
        if(obj->IsCannon())
        {
            ...
            if(turret->m_Place < 0) ERROR_E; //Место не найдено
        }
        obj = obj->GetNextLogic();
    }
}
