...
int CMatrixMap::ReloadDynamics(CStorage& stor, CMatrixMap::EReloadStep step, CBuf* robots)
{
    ...
    if(step == RS_SIDEAI)
    {
        ...
        ic = propkey->FindAsWStr(DATA_REINFORCEMENTS_TIME);
        if(ic >= 0)
        {
            m_ReinforcementsPercent = propval->GetAsWStr(ic).GetInt();
        }
                ...
                for(int s = 0; s < sc; ++s)
                {
                    if(bps->ParGet(s).GetStrPar(0, L",") == na)
                    {
                        ...
                        for(int d = 0; d < cd; d += 2)
                        {
                            ...
                            else if(na == L"SK") su->m_StrengthMul = da.GetFloat();
                            else if(na == L"DK") su->m_DangerMul = da.GetFloat();
                            else if(na == L"WRK") su->m_WaitResMul = da.GetFloat();
                            else if(na == L"BK") su->m_BraveMul = da.GetFloat();
...
DTRACE();
    ...
    if(propkey == nullptr) return -1;
    if(propval == nullptr) return -1;
...
void CMatrixMap::InitObjectsLights(void)
{
	...
	for(; sb < se; ++sb)
    {
        if((*sb)->GetObjectType() == OBJECT_TYPE_MAPOBJECT)
        {
            ((CMatrixMapObject*)(*sb))->m_Graph->InitLights(CMatrixEffect::GetSingleBrightSpriteTex(SPR_SPOT_LIGHT));
        }
    }
}
