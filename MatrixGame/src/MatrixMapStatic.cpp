...
DTRACE();
    ...
    if(tex)
    {
        ...
        if(vo->IsNoSkin(0))
        {
            voa->Init(vo, CMatrixEffect::GetSingleBrightSpriteTex(SPR_SPOT_LIGHT), CSkinManager::GetSkin(tex, gsp));
        }
        else
        {
            voa->Init(vo, CMatrixEffect::GetSingleBrightSpriteTex(SPR_SPOT_LIGHT));
        }
    }
    else
    {
...
DTRACE();
    ...
    else m_Core->m_TerainColor = 0xFFFFFFFF;
...
#define RENDSZ 512
    ...
    if(FLAG(g_Flags, GFLAG_STENCILAVAILABLE)) { ASSERT_DX(g_D3DD->Clear(0, nullptr, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER | D3DCLEAR_STENCIL, 0, 1.0f, 0)); }
    else { ASSERT_DX(g_D3DD->Clear(0, nullptr, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, 0, 1.0f, 0)); }
    ...
    float distance = height + g_Config.PortionInDiapason(height, 10.0f, 50.0f, 12.0f, -22.0f); //Чем больше, тем дальше

    //Судя по всему, старый способ коррекции ракурса по модулям
    /*
    if(fff)
    {
        height += fff[0];
    ...
    */
    ...
    //Коррекция камеры до выноса конфигов модулей в data.txt
    /*
    for(int i = 0; i < robot->m_ModulesCount; ++i)
    {
        if(robot->m_Module[i].m_Type == MRT_CHASSIS)
        {
            switch(robot->m_Module[i].m_Kind)
            {
    ...
    for(int i = 0; i < robot->m_ModulesCount; ++i)
    {
        if(robot->m_Module[i].m_Type == MRT_HULL)
        {
            ...
            case RUK_HULL_BIREX:
                ...
                distance += 2.5f;
                break;
            case RUK_HULL_DIPLOID:
                height += 9.0f;
                distance += 5.5f;
                break;
            case RUK_HULL_PARAGON:
                height += 5.5f;
                distance += 3.0f;
                break;
            case RUK_HULL_TRIDENT:
                height += 10.0f;
                distance += 5.5f;
                break;
            case RUK_HULL_FULLSTACK:
                height += 13.5;
                distance += 6;
                break;
            }
    ...
    */
    ...
    robot->m_Module[0].m_Graph->FirstFrame();
	Draw();
...
DTRACE();
    ...
    for(int i = objects_left; i < objects_rite; ++i) objects[i]->RecalcTerainColor();
}
...
DTRACE();
    ...
    for(int i = objects_left; i < objects_rite; ++i) objects[i]->Draw();
}
