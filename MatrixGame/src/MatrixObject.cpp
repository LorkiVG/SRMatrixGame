...
void CMatrixMapObject::Draw(void)
{
    ...
    bool ordinal = (m_BehaviorFlag != BEHF_BURN) || (m_BurnSkinVis == 0);
    if (ordinal)
    {
        m_Graph->Draw(0);
    }
    ...
    {
        m_Graph->DrawLights(false, m_Core->m_Matrix, nullptr);
    }
...
void CMatrixMapObject::LogicTact(int ms)
{
DTRACE();
    ...
    else if(m_BehaviorFlag == BEHF_SPAWNER)
    {
        ...
        else if(m_PrevStateRobotsInRadius == 1)
        {
            ...
            if(m_Graph->IsAnimEnd())
            {
                ...
                else robot = g_MatrixMap->Rnd(0, SRobotTemplate::m_AIRobotTypeList[ROBOTS_SPAWNER_TEMPLATES].size() - 1);

                SRobotTemplate bot = SRobotTemplate::m_AIRobotTypeList[ROBOTS_SPAWNER_TEMPLATES][robot];
