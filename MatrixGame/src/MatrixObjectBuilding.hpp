...
class CBuildQueue : public CMain
{
    int                 m_Items;
    int                 m_Timer;
...
public:
    ...
    CBuildQueue(CMatrixBuilding* base) :m_ParentBase(base) { m_Top = nullptr; m_Bottom = nullptr; m_Items = 0; m_Timer = 0; }
    ~CBuildQueue();
};
...
public:
    ...
    bool CanBeCaptured(void) const { return !FLAG(m_ObjectFlags, BUILDING_CAPTURE_IN_PROGRESS); }
    bool IsCaptured(void) const { return FLAG(m_ObjectFlags, BUILDING_CAPTURE_IN_PROGRESS); }
    ...
    int GetRobotsBuildingQueueCount(void) const { return m_BS.GetRobotsCnt(); }
    ...
    bool IsSpawningUnit(void) const { return FLAG(m_ObjectFlags, BUILDING_SPAWNING_UNIT); };
	...
	virtual void GetResources(dword need); //Запрашиваем необходимые для данного объекта ресурсы
