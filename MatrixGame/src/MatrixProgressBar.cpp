...
void CMatrixProgressBar::KillClone(EPBCoord pbc)
{
    if(pbc < m_CoordCount)
    {
        ...
        else if(oldc > m_CoordCount) m_Coord = (SPBPos*)HAllocEx(m_Coord, sizeof(SPBPos) * m_CoordCount, g_MatrixHeap);
    }
}
...
void CMatrixProgressBar::DrawAll(void)
{
    ...
    if (m_First == nullptr && m_FirstClones == nullptr) return;
...
void CMatrixProgressBar::DrawAllClones(void)
{
    ...
    if (m_FirstClones == nullptr) return;
