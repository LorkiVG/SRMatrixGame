...
void CMatrixSideUnit::InterpolateArcadedRobotHullPos(int ms)
{
    //return;
    if(!m_ArcadedP_available) return;
...
D3DXVECTOR3 CMatrixSideUnit::CorrectWalkingArcadedRobotHullPos(D3DXVECTOR3& p, CMatrixRobot* r)
{
...
DTRACE();
    ...
    for(int i = 0; i < MAX_TEAM_CNT; ++i)
    {
        ...
        m_Team[i].m_RoadPath = HNew(g_MatrixHeap) CMatrixRoadRoute(&(g_MatrixMap->m_RoadNetwork));
    }
    ...
    for(int i = 0; i < MAX_LOGIC_GROUP; ++i)
    {
        m_PlayerGroup[i].m_RoadPath = HNew(g_MatrixHeap) CMatrixRoadRoute(&(g_MatrixMap->m_RoadNetwork));
    }
...
CMatrixSideUnit::~CMatrixSideUnit()
{
    ...
    if(m_Constructor != nullptr) HDelete(CConstructor, m_Constructor, g_MatrixHeap);
...
{
    DTRACE();
    if(!m_PlaceList) m_PlaceList = (int*)HAllocClear(sizeof(int) * g_MatrixMap->m_RoadNetwork.m_PlaceCnt, g_MatrixHeap);
}
...
DTRACE();
    ...
    fl->m_FlyerKind = flyer_type;
    fl->InitMaxHitpoint(flyer_structure);

    g_MatrixMap->m_AD_Obj[g_MatrixMap->m_AD_Obj_cnt] = fl;
    ++g_MatrixMap->m_AD_Obj_cnt;

    fl->ApplyOrder(to, m_Id, order, ang, place, bpos, robot_template);
}
...
DTRACE();
    ...
    if(m_Id == PLAYER_SIDE && IsRobotArcadeMode() && g_IFaceList->m_InFocus != INTERFACE)
    {
...
void CMatrixSideUnit::OnRButtonUp(const CPoint&)
{
    ...
    if(IsArcadeMode())
    {
        if(GetArcadedObject()->IsRobotAlive())
        {
            CMatrixRobotAI* robot = GetArcadedObject()->AsRobot();
...
{
    if(!IsRobotArcadeMode() || !m_Arcaded) return;

    CMatrixRobotAI* robot = m_Arcaded->AsRobot();
...
bool CMatrixSideUnit::IsRobotArcadeMode()
{
    DTRACE();
...
void CMatrixSideUnit::SetArcadedObject(CMatrixMapStatic* o)
{
    ...
    if(IsRobotArcadeMode() && !m_Arcaded->IsDIP())
    {
        CMatrixRobotAI* robot = m_Arcaded->AsRobot();
    ...
    else if(IsRobotArcadeMode() && o->GetObjectType() == OBJECT_TYPE_ROBOTAI)
    {
        CMatrixRobotAI* robot = m_Arcaded->AsRobot();
        if(g_IFaceList) g_IFaceList->DeleteWeaponDynamicStatics();
        robot->UnSelect();
    ...
    else if(IsRobotArcadeMode() && o->GetObjectType() == OBJECT_TYPE_FLYER)
    {
        CMatrixRobotAI* robot = m_Arcaded->AsRobot();
        if(g_IFaceList) g_IFaceList->DeleteWeaponDynamicStatics();
        robot->UnSelect();
    ...
    if(IsRobotArcadeMode())
    {
        CMatrixRobotAI* robot = o->AsRobot();
...
DTRACE();
    ...
    CMatrixGroup* new_group = HNew(g_MatrixHeap) CMatrixGroup;
...
void CMatrixSideUnit::TactHL()
{
    ...
    if(m_Region == nullptr) m_Region = (SMatrixLogicRegion*)HAllocClear(sizeof(SMatrixLogicRegion) * g_MatrixMap->m_RoadNetwork.m_RegionCnt, g_MatrixHeap);
    if(m_RegionIndex == nullptr) m_RegionIndex = (int*)HAllocClear(sizeof(int) * g_MatrixMap->m_RoadNetwork.m_RegionCnt, g_MatrixHeap);
            ...
            if(((CMatrixRobotAI*)ms)->m_CurrState != ROBOT_DIP)
            {
                ...
                if(ms->AsRobot()->m_Side == m_Id)
                {
                    ...
                    if(team >= 0 && team < m_TeamCnt)
                    {
                        ...
                        m_Team[team].OrMove(1 << (ms->AsRobot()->m_Module[0].m_Kind - 1));
    ...
    */
    ...
    //Строим робота
    if(SRobotTemplate::m_AIRobotTypeList[m_Id].size()) ChooseAndBuildAIRobot(m_Id); //Если имеются шаблоны роботов конкретно для текущей стороны, выбираем из них
    ...
    for(int g = 0; g < MAX_LOGIC_GROUP; ++g)
    {
        ...
        while(obj)
        {
            ...
            if(obj->IsRobotAlive() && obj->GetSide() == m_Id)
            {
                ...
                {
                    SMatrixPlace *place = g_MatrixMap->m_RoadNetwork.GetPlace(obj->AsRobot()->GetEnv()->m_Place);
        ...
        */
            ...
            for(i = 0; i < g_MatrixMap->m_RoadNetwork.m_RegionCnt; ++i)
            {
                CPoint tp=g_MatrixMap->m_RoadNetwork.m_Region[i].m_Center;
                CHelper::Create(10,5001)->Cone(D3DXVECTOR3(GLOBAL_SCALE_MOVE*tp.x,GLOBAL_SCALE_MOVE*tp.y,0),D3DXVECTOR3(GLOBAL_SCALE_MOVE*tp.x,GLOBAL_SCALE_MOVE*tp.y,20),1.0f,1.0f,g_MatrixMap->m_RoadNetwork.m_Region[i].m_Color,g_MatrixMap->m_RoadNetwork.m_Region[i].m_Color,5);
                for(u = 0; u < i; ++u)
                {
                    CHelper::Create(10,5001)->Sphere(D3DXVECTOR3(GLOBAL_SCALE_MOVE*tp.x,GLOBAL_SCALE_MOVE*tp.y,20.0f+8.0f*(u+1)),1.0f,3,g_MatrixMap->m_RoadNetwork.m_Region[i].m_Color);
                }
                CHelper::Create(10,5001)->Cone(D3DXVECTOR3(GLOBAL_SCALE_MOVE*tp.x,GLOBAL_SCALE_MOVE*tp.y,0),D3DXVECTOR3(GLOBAL_SCALE_MOVE*tp.x,GLOBAL_SCALE_MOVE*tp.y,40),GLOBAL_SCALE_MOVE*g_MatrixMap->m_RoadNetwork.m_Region[i].m_RadiusPlace,GLOBAL_SCALE_MOVE*g_MatrixMap->m_RoadNetwork.m_Region[i].m_RadiusPlace,g_MatrixMap->m_RoadNetwork.m_Region[i].m_Color,g_MatrixMap->m_RoadNetwork.m_Region[i].m_Color,20);
            }
            */
#endif
}
...
float CMatrixSideUnit::BuildRobotMinStrange(CMatrixBuilding* base)
{
    ...
    while(mps)
    {
        if(mps->IsRobotAlive())
        {
            ...
            i = GetRegion(robot);
            if(i == baseregion || g_MatrixMap->m_RoadNetwork.IsNearestRegion(i, baseregion))
            {
...
void CMatrixSideUnit::ChooseAndBuildAIRobot(int cur_side)
{
        ...
        if(mps->GetSide() == m_Id && mps->IsBuildingAlive() && mps->IsBase())
        {
            ...
            while(true)
            {
                if(!base)
                {
                    ...
                    min_strange = BuildRobotMinStrange(mps->AsBuilding());
                }
                else
                {
                    i = GetRegion(mps);
    ...
    wait_time = Float2Int(m_WaitResMul * float(min(wait_time, 40000))); // Нет смысла долго ждять.
    ...
    if(m_WaitResForBuildRobot >= 0)
    {
        ...
        {
            k = (SRobotTemplate::m_AIRobotTypeList[cur_side][m_WaitResForBuildRobot].m_Resources[r] - m_Resources[r]);
            if(k > 0)
            {
                if(wr[r] <= 0 && base_cnt <= 0)
                {
                    wait_end = 1000000000;
                    break;
                }
                wait_end = max(wait_end, Float2Int(float(k * g_Config.m_Timings[r]) / float(wr[r] * RESOURCES_INCOME + (RESOURCES_INCOME_BASE * GetResourceForceUp() / 100 * base_cnt))));
            }
        }
    }
    ...
    {
        /*
        if(m_Resources[r] < mr) //Не ждём ресурса, которого слишком мало
        {
            wr[r] = m_Resources[r];
        }
        else
        */
        wr[r] = (RESOURCES_INCOME * wr[r] + (RESOURCES_INCOME_BASE * GetResourceForceUp() / 100 * base_cnt)) * (wait_time / g_Config.m_Timings[r]) + m_Resources[r];
    }

    //Создаём список роботов, которых можем построить сразу, и список тех, которых построим, если немного подождём
    int* list = (int*)HAlloc(SRobotTemplate::m_AIRobotTypeList[cur_side].size() * sizeof(int) * 2, g_MatrixHeap);
    int* lwait = list + SRobotTemplate::m_AIRobotTypeList[cur_side].size();
    cnt = 0;
    lw_cnt = 0;

    bool no_bomb = ((m_BuildRobotLast >= 0) && SRobotTemplate::m_AIRobotTypeList[cur_side][m_BuildRobotLast].m_HaveBomb)   ||
                   ((m_BuildRobotLast2 >= 0) && SRobotTemplate::m_AIRobotTypeList[cur_side][m_BuildRobotLast2].m_HaveBomb) ||
                   ((m_BuildRobotLast3 >= 0) && SRobotTemplate::m_AIRobotTypeList[cur_side][m_BuildRobotLast3].m_HaveBomb);

    if(!no_bomb)
    {
        if((g_MatrixMap->GetTime() - m_TimeLastBomb) < m_TimeNextBomb) no_bomb = true;
    }

    bool no_repair = ((m_BuildRobotLast >= 0) && SRobotTemplate::m_AIRobotTypeList[cur_side][m_BuildRobotLast].m_HaveRepair)   ||
                     ((m_BuildRobotLast2 >= 0) && SRobotTemplate::m_AIRobotTypeList[cur_side][m_BuildRobotLast2].m_HaveRepair) ||
                     ((m_BuildRobotLast3 >= 0) && SRobotTemplate::m_AIRobotTypeList[cur_side][m_BuildRobotLast3].m_HaveRepair);

    //for(int bt = 0; bt <= 1; ++bt)
    //{
    for(i = 0; i < SRobotTemplate::m_AIRobotTypeList[cur_side].size(); ++i)
    {
        if(no_bomb && SRobotTemplate::m_AIRobotTypeList[cur_side][i].m_HaveBomb) continue;
        if(no_repair && SRobotTemplate::m_AIRobotTypeList[cur_side][i].m_HaveRepair) continue;
        if(!no_repair && !SRobotTemplate::m_AIRobotTypeList[cur_side][i].m_HaveRepair) continue;
        if(SRobotTemplate::m_AIRobotTypeList[cur_side][i].m_Strength < min_strange) continue; // Сила должна быть больше минимальной

        for(r = 0; r < MAX_RESOURCES; ++r) if (m_Resources[r] < SRobotTemplate::m_AIRobotTypeList[cur_side][i].m_Resources[r]) break;

        if(r >= MAX_RESOURCES)
        {
            //Одинаковых роботов не строим, так как не разнообразно и некрасиво
            if(/* !bt && */ i != m_WaitResForBuildRobot && m_BuildRobotLast >= 0 && SRobotTemplate::m_AIRobotTypeList[cur_side][i].DifWeapon(SRobotTemplate::m_AIRobotTypeList[cur_side][m_BuildRobotLast]) > 0.6);
            else if(/* !bt && */ i != m_WaitResForBuildRobot && m_BuildRobotLast2 >= 0 && SRobotTemplate::m_AIRobotTypeList[cur_side][i].DifWeapon(SRobotTemplate::m_AIRobotTypeList[cur_side][m_BuildRobotLast2]) > 0.8);
            //else if(/* !bt && */ i != m_WaitResForBuildRobot && m_BuildRobotLast3 >= 0 && SRobotTemplate::m_AIRobotTypeList[cur_side][i].DifWeapon(SRobotTemplate::m_AIRobotTypeList[cur_side][m_BuildRobotLast3]) > 0.9);
            else
            {
                list[cnt] = i;
                ++cnt;
        ...
        {
            for(r = 0; r < MAX_RESOURCES; ++r) if(wr[r] < SRobotTemplate::m_AIRobotTypeList[cur_side][i].m_Resources[r]) break;
            if(r >= MAX_RESOURCES)
            {
                //Одинаковых роботов не строим, так как не разнообразно и некрасиво
                if(/* !bt && */ i != m_WaitResForBuildRobot && m_BuildRobotLast >= 0 && SRobotTemplate::m_AIRobotTypeList[cur_side][i].DifWeapon(SRobotTemplate::m_AIRobotTypeList[cur_side][m_BuildRobotLast]) > 0.6);
    ...
    if(m_WaitResForBuildRobot >= 0)
    {
        ...
        if(i < cnt)
        {
            ...
            {
                for(r = 0; r < MAX_RESOURCES; ++r) m_Resources[r] = max(0, m_Resources[r] - SRobotTemplate::m_AIRobotTypeList[cur_side][list[i]].m_Resources[r]);
                m_Constructor->SetBase(base);
                m_Constructor->BuildRobotByTemplate(SRobotTemplate::m_AIRobotTypeList[cur_side][list[i]]);

                if(SRobotTemplate::m_AIRobotTypeList[cur_side][list[i]].m_HaveBomb) m_TimeLastBomb = g_MatrixMap->GetTime();

                m_BuildRobotLast3 = m_BuildRobotLast2;
                m_BuildRobotLast2 = m_BuildRobotLast;
                m_BuildRobotLast = list[i];

                DMSide(L"BuildRobot BuildWaitRobotStrange=<f>", SRobotTemplate::m_AIRobotTypeList[cur_side][list[i]].m_Strength);
                HFree(list, g_MatrixHeap);
            }
            m_WaitResForBuildRobot = -1;
            return;
    ...
    {
        DMSide(L"BuildRobot CancelWaitRobotStrange=<f>", SRobotTemplate::m_AIRobotTypeList[cur_side][m_WaitResForBuildRobot].m_Strength);
        m_WaitResForBuildRobot = -1;
    }
    ...
    DMSide(L"BuildRobot MinStrange=<f>", min_strange);
    DMSide(L"BuildRobot ImmediatelyCnt=<i> WaitCnt=<i>", cnt, lw_cnt);

    //Стоит ли ожидать
    if(cnt > 0 && lw_cnt > 0 && SRobotTemplate::m_AIRobotTypeList[cur_side][lwait[0]].m_Strength * 0.6 > SRobotTemplate::m_AIRobotTypeList[cur_side][list[0]].m_Strength)
    {
        //Выбираем из лучших, случайно по приоритету
        for(i = 1; i < lw_cnt; ++i) if(SRobotTemplate::m_AIRobotTypeList[cur_side][lwait[i]].m_Strength < 0.7 * SRobotTemplate::m_AIRobotTypeList[cur_side][lwait[0]].m_Strength) break;
        lw_cnt = i;

        for(r = 0; r < MAX_RESOURCES; ++r) if(m_Resources[r] < mr) break;
        if(r < MAX_RESOURCES) //Сортируем по ресурсам, которых мало
        {
            for(i = 0; i < lw_cnt - 1; ++i)
            {
                ik = 0;
                for(r = 0; r < MAX_RESOURCES; ++r) if(m_Resources[r] < mr) ik += SRobotTemplate::m_AIRobotTypeList[cur_side][lwait[i]].m_Resources[r];

                for(u = i + 1; u < lw_cnt; ++u)
                {
                    uk = 0;
                    for(r = 0; r < MAX_RESOURCES; ++r) if(m_Resources[r] < mr) uk += SRobotTemplate::m_AIRobotTypeList[cur_side][lwait[u]].m_Resources[r];
            ...
            ik = 0;
            for(r = 0; r < MAX_RESOURCES; ++r) if(m_Resources[r] < mr) ik += SRobotTemplate::m_AIRobotTypeList[cur_side][lwait[0]].m_Resources[r];
            ik += ik / 10;
            for(i = 1; i < lw_cnt; ++i)
            {
                uk = 0;
                for(r = 0; r < MAX_RESOURCES; ++r) if(m_Resources[r] < mr) uk += SRobotTemplate::m_AIRobotTypeList[cur_side][lwait[i]].m_Resources[r];
                if(uk > ik) break;
            }
            lw_cnt = i;
        }
        ...
        k = 0;
        for(i = 0; i < lw_cnt; ++i) k += SRobotTemplate::m_AIRobotTypeList[cur_side][lwait[i]].m_TemplatePriority;
        k = g_MatrixMap->Rnd(0, k - 1);

        for(i = 0; i < lw_cnt; ++i)
        {
            k -= SRobotTemplate::m_AIRobotTypeList[cur_side][lwait[i]].m_TemplatePriority;
            if(k < 0) break;
        }
        if(i >= lw_cnt) ERROR_E;

        m_WaitResForBuildRobot = lwait[i];
        DMSide(L"BuildRobot WaitRobotStrange=<f>", SRobotTemplate::m_AIRobotTypeList[cur_side][m_WaitResForBuildRobot].m_Strength);
    }
    else if(cnt > 0) //Строим сразу
    {
        //Выбираем из лучших, случайно по приоритету
        for(i = 1; i < cnt; ++i) if(SRobotTemplate::m_AIRobotTypeList[cur_side][list[i]].m_Strength < 0.7 * SRobotTemplate::m_AIRobotTypeList[cur_side][list[0]].m_Strength) break;
        cnt = i;
        ...
        if(r < MAX_RESOURCES) //Сортируем по ресурсам, которых мало
        {
            ...
            {
                ik = 0;
                for(r = 0; r < MAX_RESOURCES; ++r) if(m_Resources[r] < mr) ik += SRobotTemplate::m_AIRobotTypeList[cur_side][list[i]].m_Resources[r];

                for(u = i + 1; u < cnt; ++u)
                {
                    uk = 0;
                    for(r = 0; r < MAX_RESOURCES; ++r) if(m_Resources[r] < mr) uk += SRobotTemplate::m_AIRobotTypeList[cur_side][list[u]].m_Resources[r];
            ...
            ik = 0;
            for(r = 0; r < MAX_RESOURCES; ++r) if(m_Resources[r] < mr) ik += SRobotTemplate::m_AIRobotTypeList[cur_side][list[0]].m_Resources[r];
            ik += ik / 10;
            for(i = 1; i < cnt; ++i)
            {
                uk = 0;
                for(r = 0; r < MAX_RESOURCES; ++r) if(m_Resources[r] < mr) uk += SRobotTemplate::m_AIRobotTypeList[cur_side][list[i]].m_Resources[r];
                if(uk > ik) break;
            }
            cnt = i;
        }
        ...
        k = 0;
        for(i = 0; i < cnt; ++i) k += SRobotTemplate::m_AIRobotTypeList[cur_side][list[i]].m_TemplatePriority;
        k = g_MatrixMap->Rnd(0, k - 1);

        for(i = 0; i < cnt; ++i)
        {
            k -= SRobotTemplate::m_AIRobotTypeList[cur_side][list[i]].m_TemplatePriority;
            if(k < 0) break;
        }
        if(i >= cnt) ERROR_E;
        ...
        if(base->m_BS.GetItemsCnt() < 6)
        {
            ...
            DMSide(L"BuildRobot BuildRobotStrange=<f>", SRobotTemplate::m_AIRobotTypeList[cur_side][list[i]].m_Strength);
        }
    }
...
void CMatrixSideUnit::ChooseAndBuildAICannon(void)
{
    ...
    while(mps)
    {
        ...
        {
            ASSERT(((CMatrixCannon*)mps)->m_Num >= 1 && ((CMatrixCannon*)mps)->m_Num <= TURRET_KINDS_TOTAL);
            ++ct[((CMatrixCannon*)mps)->m_TurretKind];
        }
    ...
    if(turret_data->cost_titan > m_Resources[0]) return;
    if(turret_data->cost_electronics > m_Resources[1]) return;
    if(turret_data->cost_energy > m_Resources[2]) return;
    if(turret_data->cost_plasma > m_Resources[3]) return;
    ...
    m_Resources[0] -= turret_data->cost_titan;
    m_Resources[1] -= turret_data->cost_electronics;
    m_Resources[2] -= turret_data->cost_energy;
    m_Resources[3] -= turret_data->cost_plasma;
...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
void CMatrixSideUnit::TactPL(int onlygroup)
{
DTRACE();
            ...
            while(obj)
            {
                if(obj->IsRobotAlive() && obj->GetSide() == m_Id && obj->AsRobot()->GetGroupLogic() == i)
                {
                    ...
                    if(robot->HaveBomb())
                    {
                        ...
                        else if(m_PlayerGroup[i].m_Obj && m_PlayerGroup[i].m_Obj->IsAlive() && D3DXVec2LengthSq(&(GetWorldPos(robot) - GetWorldPos(m_PlayerGroup[i].m_Obj))) < POW2(150))
                        {
                            robot->BigBoom();
                            --t;
                        }
...
bool CMatrixSideUnit::FirePL(int group)
{
    ...
    for(i = 0; i < rlcnt; ++i)
    {
        ...
        if(!(env->m_TargetAttack && env->SearchEnemy(env->m_TargetAttack)))
        {
            ...
            if(enemyfind) env->m_TargetAttack = enemyfind->m_Enemy;
        }
    }
...
void CMatrixSideUnit::PGAssignPlace(int no, CPoint& center)
{
    ...
    int growsizemax=50;
    g_MatrixMap->m_RoadNetwork.ActionDataPL(CRect(center.x-growsizemax,center.y-growsizemax,center.x+growsizemax,center.y+growsizemax),0);

    CRect plr=g_MatrixMap->m_RoadNetwork.CorrectRectPL(CRect(center.x-growsizemax,center.y-growsizemax,center.x+growsizemax,center.y+growsizemax));
    ...
    while(obj)
    {
        if(obj->IsRobotAlive() && (obj->AsRobot()->GetSide()==m_Id && obj->AsRobot()->GetGroupLogic()==no))
        {
            ...
            {
                g_MatrixMap->m_RoadNetwork.GetPlace(robot->GetEnv()->m_Place)->m_Data=1;
            }
        }
...
void CMatrixSideUnit::PGCalcStat()
{
    ...
    for (i = 0; i < g_MatrixMap->m_RoadNetwork.m_RegionCnt; ++i) m_Region[i].m_Danger += m_Region[i].m_DangerAdd;
}
...
inline bool CanMove(byte movetype, CMatrixRobotAI* robot)
{
    return !(movetype & (1 << (robot->m_Module[0].m_Kind - 1)));
}
