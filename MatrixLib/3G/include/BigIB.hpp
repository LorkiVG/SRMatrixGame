...
private:
     // do not modify these!!!
    int one = -1;    // index of SBigIBOne in SBigIB
    int part = 0;   // index of part in SBigIBOne
    int base = 0;   // index of vert in SBigIBOne

public:
    friend class CBigIB;
    friend struct SBigIBOne;

    SBigIBSource() = default;
...
struct SBigIBPart
{
    SBigIBSource* source = nullptr;
    dword         flags = 0;
};

struct SBigIBOne
{
    SBigIBPart* parts = nullptr;
    int         count = 0;
    int         noneedcount = 0;
    int         ibsize = 0; // in bytes
    D3D_IB      ib;
...
class CBigIB : public Base::CMain
{
    ...
    SBigIBOne* m_IB_array = nullptr;
    int        m_IB_count = 0;
    //int      m_sz = 0; // 2 or 4

    int m_LastOne = -1;

    CBigIB(CHeap* heap) : CMain(), m_Heap(heap) {};
    ~CBigIB() = default;
...
__inline bool CBigIB::DelSource(SBigIBSource *src) // return true if BigIB has destroyed
{
    ...
    if (nn > 0)
    {
        ...
        if ((nn + m_IB_array[src->one].noneedcount) == m_IB_array[src->one].count)
        {
            ...
            ASSERT(m_IB_array[src->one].ib != nullptr); 
