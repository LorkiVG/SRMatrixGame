...
//Пример: глаза "Блокиратора", свет в груди "Фулстека"
    ...
    if(m_LightsCount)
    {
        ...
        for(int m = 0, l = 0; m < m_VO->GetMatrixCount(); ++m)
        {
            ...
            if(matrix_name[0] == '$')
            {
                ...
                float ra = li_info.GetDoublePar(0, L",");
...
bool CVectorObjectAnim::Tact(int cms, float speed_factor)
{
DTRACE();
        ...
        if(m_Lights[i].period > 0)
        {
            ...
            for(int ci = 0; ci < m_Lights[i].intervals_cnt; ++ci)
            {
                ...
                if(m_Lights[i].time >= cis->time1 && m_Lights[i].time <= cis->time2)
                {
                    ...
                    if(!delta) m_Lights[i].BB().SetColor(cis->c1);
                    else
                    {
    ...
    while(m_Time > m_TimeNext) //Если пришло время обновить фрейм
    {
        ++m_Frame;
        if(m_AnimLooped)
        {
            if(m_Frame >= frames_count) m_Frame = 0;
        }
        else
        {
            if(m_Frame >= frames_count)
            {
                m_TimeNext += 1000;
                m_Frame = frames_count - 1;
                break;
            }
        }
...
DTRACE();
    ...
    {
        if(m_Frame < frames_count - 1) ++m_Frame;
...
dword CVectorObjectAnim::PrevFrame(void)
{
DTRACE();
    ...
    {
        --m_Frame;
        if(m_Frame <= 0) m_Frame = m_VO->GetAnimFramesCount(m_Anim) - 1;
    }
    else
    {
...
DTRACE();
    ...
    while(u)
    {
        if(u->m_Obj) u->m_Obj->DrawLights(now, u->m_MatrixWorld, iview);
        u = u->m_Next;
    }
}
