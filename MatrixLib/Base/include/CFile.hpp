...
public:
	CFile(CHeap* heap = nullptr);
	CFile(const CWStr& filename, CHeap* heap = nullptr);
	CFile(const wchar* filename, CHeap* heap = nullptr);
	CFile(const wchar* filename, int len, CHeap* heap = nullptr);
	~CFile();
	...
	bool OpenRead(dword shareMode = FILE_SHARE_READ, bool crash_on_fail_to_load = true); // FILE_SHARE_DELETE FILE_SHARE_READ FILE_SHARE_WRITE
    ...
    static bool FileExist(CWStr& outname, const wchar* mname, const wchar* exts = nullptr, bool withpar = false);
};
