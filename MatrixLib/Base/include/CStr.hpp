...
class BASE_API CStr : public CMain
{
    ...
    public:
		CStr(void) :CMain(), m_Heap(nullptr), m_MaxLen(0), m_Len(0), m_Str(nullptr) {}
		explicit CStr(CHeap* heap) : CMain(), m_Heap(heap), m_MaxLen(0), m_Len(0), m_Str(nullptr) {}
		CStr(const CStr& s, CHeap* heap = nullptr) : CMain() { Init(s.m_Str, s.Len(), heap ? heap : s.m_Heap); }
		explicit CStr(const CWStr& s, CHeap* heap = nullptr);
		explicit CStr(const wchar* s, CHeap* heap = nullptr);
		explicit CStr(const char* s, CHeap* heap = nullptr) : CMain() { Init(s, int(strlen(s)), heap); }
		CStr(const char* s, int len, CHeap* heap = nullptr) : CMain() { Init(s, len, heap); }
		explicit CStr(char sim, CHeap* heap = nullptr) : CMain(), m_Heap(heap), m_MaxLen(16), m_Len(1) { m_Str = (char*)HAlloc(uint(m_MaxLen + 1), m_Heap); m_Str[0] = sim; m_Str[1] = 0; };
		CStr(char sim, int count, CHeap* heap = nullptr) :CMain(), m_Heap(heap), m_MaxLen(count + 16), m_Len(count) { m_Str = (char*)HAlloc(uint(m_MaxLen + 1), m_Heap); memset(m_Str, int(sim), uint(count)); m_Str[count] = 0; };
		...
		void ClearFull(void) { if(m_Str!=nullptr) { HFree(m_Str,m_Heap); m_Str=nullptr; } m_Len=m_MaxLen=0; }
		...
		char* GetBufEx(void) const			    { if(IsEmpty()) return nullptr; return m_Str; }
