...
bool CHsFolder::ReadFolder(dword Handle, dword Offset)
{
    ...
    SetFilePointer((HANDLE)Handle, Offset, nullptr, FILE_BEGIN);
    BOOL res = ::ReadFile((HANDLE)Handle, &m_FolderRec, sizeof(SFolderRec), &temp, nullptr);
    ...
    res = ::ReadFile((HANDLE)Handle, m_Files, sz, &temp, nullptr);
    if(res == FALSE || temp != sz)
    {
        for(dword i = 0; i < m_FolderRec.m_Recnum; ++i) m_Files[i].m_Extra = 0;
        Clear();
        return false;
    }
...
void  CHsFolder::UpdateFileRec(void)
{
    if (m_Parent)
    {
        ...
        if (PFile == nullptr) ERROR_S(L"Packet file system error!");
...
void   CHsFolder::ListFileNames(FILENAME_CALLBACK_FUNC Func)
{
    ...
    for (dword i=0; i<m_FolderRec.m_Recnum; ++i)
    {
        ...
        if (PFile->m_Free == 0)
        {
            ...
            {
                CHsFolder *PFolder = (CHsFolder *)PFile->m_Extra;
                if (PFolder==nullptr) ERROR_S(L"Pack file system error!");
...
CPackFile::CPackFile(CHeap* heap, const wchar* name) : m_Heap(heap), m_FileName(name, heap)
{
    ...
    m_RootOffset = 0;
    //m_ID = 0xFFFFFFFF;
    for(int i = 0; i < MAX_VIRTUAL_HANDLE_COUNT; ++i) m_Handles[i].m_Free = true;
}
...
{
    if(m_Handle != 0xFFFFFFFF || m_RootFolder != nullptr) ClosePacketFile();
    ...
    if(IS_UNICODE()) m_Handle = (dword)CreateFileW(m_FileName.Get(), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
    else m_Handle = (dword)CreateFileA(CStr(m_FileName, m_Heap).Get(), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
    ...
    dword temp = 0;
    if(!::ReadFile((HANDLE)m_Handle, &m_RootOffset, 4, &temp, nullptr))
    {
#ifdef RAISE_ALL_EXCEPTIONS
        ERROR_S((L"Error openning package file [READ]:" + m_FileName).Get());
#endif
...
{
    if(m_Handle == 0xFFFFFFFF && m_RootFolder == nullptr) return false;
    Clear();
    if(m_RootFolder)
    {
        HDelete (CHsFolder, m_RootFolder, m_Heap);
    ...
    bool res;
    if(m_Handle != 0xFFFFFFFF) res = CloseHandle((HANDLE)m_Handle) != FALSE;
    else res = true;
    m_Handle = 0xFFFFFFFF;
    return res;
}
...
bool CPackFile::CreatePacketFileEx(void)
{
    ...
    // Открываем файл для записи/чтения
    if(IS_UNICODE()) m_Handle = (dword)CreateFileW(m_FileName.Get(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0);
    else m_Handle = (dword)CreateFileA(CStr(m_FileName, m_Heap).Get(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0);
...
bool CPackFile::OpenPacketFileEx()
{
    ...
    if(m_Handle != 0xFFFFFFFF || m_RootFolder != nullptr) ClosePacketFileEx();
    ...
    // Открываем файл для записи/чтения
    if(IS_UNICODE()) m_Handle = (dword)CreateFileW(m_FileName.Get(), GENERIC_READ|GENERIC_WRITE, FILE_SHARE_READ|FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
    else m_Handle = (dword)CreateFileA(CStr(m_FileName, m_Heap).Get(), GENERIC_READ|GENERIC_WRITE, FILE_SHARE_READ|FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
...
int CPackFile::GetFreeHandle(void)
{
    for(int i = 0; i < MAX_VIRTUAL_HANDLE_COUNT; ++i)
    {
        if(m_Handles[i].m_Free) return i;
    }
    return -1;
}
...
dword CPackFile::Open(const CStr& filename, dword modeopen)
{
    ...
    if(PFile == nullptr)
    {
        ...
        m_Handles[H].m_Handle = (dword)CreateFileA(filename.Get(), modeopen, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);

        if((HANDLE)m_Handles[H].m_Handle == INVALID_HANDLE_VALUE) return 0xFFFFFFFF;
        m_Handles[H].m_StartOffset = 0;
        m_Handles[H].m_Offset = 0;
        m_Handles[H].m_Size = ::GetFileSize((HANDLE)m_Handles[H].m_Handle,nullptr);
        m_Handles[H].m_SouBuf = nullptr;
        m_Handles[H].m_DesBuf = nullptr;
        ...
        dword Error = SetFilePointer((HANDLE)m_Handles[H].m_Handle, m_Handles[H].m_Offset, nullptr, FILE_BEGIN);
        if(Error == 0xFFFFFFFF)
        {
            ERROR_S((L"File system error:" + CWStr(filename)).Get());
        }
...
bool CPackFile::Close(dword Handle)
{
    ...
    if(m_Handles[Handle].m_Handle == m_Handle)
    {
        ...
        {
            HFree(m_Handles[Handle].m_SouBuf, m_Heap); m_Handles[Handle].m_SouBuf = nullptr;
            HFree(m_Handles[Handle].m_DesBuf, m_Heap); m_Handles[Handle].m_DesBuf = nullptr;
        }
...
dword CPackFile::SetCompressedBlockPointer(dword StartOffset, int nBlock)
{
    ...
    while(true)
    {
        ::SetFilePointer((HANDLE)m_Handle, Offset, nullptr, FILE_BEGIN);
        ::ReadFile((HANDLE)m_Handle, &Size, 4, &Temp, nullptr);
        if(nBlock)
        {
            --nBlock;
            Offset += Size + 4;
...
//#endif
    ...
    if(m_Handles[Handle].m_Compressed)
    {
        ...
        while(Size)
        {
            ...
            IntBlockSize = Size;
            if(IntBlockSize > (65536 - IntBlockOffset)) IntBlockSize = 65536 - IntBlockOffset;
            ...
            if(m_Handles[Handle].m_Blocknumber != Block)
            {
                ...
                BOOL res = ::ReadFile((HANDLE)m_Handle, m_Handles[Handle].m_SouBuf, CompSize, &Temp, nullptr);
    ...
    else
    {
        ::SetFilePointer((HANDLE)m_Handles[Handle].m_Handle,
                       m_Handles[Handle].m_Offset,nullptr,FILE_BEGIN);
        BOOL res = ::ReadFile((HANDLE)m_Handles[Handle].m_Handle, buf, Size, &Temp, nullptr);
...
bool CPackFile::Write(dword Handle, const void* buf, int Size)
{
    ...
    ::SetFilePointer((HANDLE)m_Handles[Handle].m_Handle, m_Handles[Handle].m_Offset,nullptr,FILE_BEGIN);
    
    BOOL res = ::WriteFile((HANDLE)m_Handles[Handle].m_Handle, buf, Size, &Temp, nullptr);
    bool ret = (res != FALSE) && (Size == Temp);
    m_Handles[Handle].m_Offset = m_Handles[Handle].m_Offset + Temp;
...
bool  CPackFile::SetPos(dword Handle, dword Pos, int ftype)
{
    ...
    {
        Error = SetFilePointer((HANDLE)m_Handles[Handle].m_Handle, m_Handles[Handle].m_StartOffset+Pos,nullptr,FILE_BEGIN);
        if (Error==0xFFFFFFFF)
        {
...
int CPackFile::FindFirst(const CStr& path, dword Attr, SSearchRec& S)
{
    ...
    S.Name = "";
    S.Folder = nullptr;
    if(m_RootFolder == nullptr) return 0;
    if(path.IsEmpty()) S.Folder = m_RootFolder;
    else S.Folder = m_RootFolder->GetFolderEx(S.Path);
...
bool  CPackCollection::FileExists(const CStr& name)
{
    PCPackFile* ff = m_PackFiles.Buff<PCPackFile>();
    PCPackFile* fe = m_PackFiles.BuffEnd<PCPackFile>();
    for(; ff < fe; ++ff) if((*ff)->FileExists(name)) return true;
    return false;
}
...
bool  CPackCollection::PathExists(const CStr& path)
{
    PCPackFile* ff = m_PackFiles.Buff<PCPackFile>();
    PCPackFile* fe = m_PackFiles.BuffEnd<PCPackFile>();
    for(; ff < fe; ++ff) if((*ff)->PathExists(path)) return true;
    return false;
}
...
dword CPackCollection::GetPos(dword Handle)
{
    ...
    int number = Handle >> MAX_VIRTUAL_HANDLE_COUNT_BITS;
    PCPackFile P = GetPacketFile(number);
    if(P == nullptr) return 0xFFFFFFFF;
...
bool CPackCollection::UnpackFile(const CStr& souname, const CStr& desname)
{
    ...
    for(; ff < fe; ++ff)
    {
        if((*ff)->FileExists(souname)) return (*ff)->UnpackFile(souname, desname);
    }
    return false;
}
*/
