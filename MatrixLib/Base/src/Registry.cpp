...
HKEY Reg_CreateKey(HKEY key, const wchar * path, dword access)
{
	...
	if(GetVersion()<0x80000000) rz=RegCreateKeyExW(key,path,0,nullptr,0,access,nullptr,&kkey,&dv);
	else rz=RegCreateKeyExA(key,CStr(CWStr(path)).Get(),0,nullptr,0,access,nullptr,&kkey,&dv);
...
bool Reg_GetData(HKEY key,const wchar * name,dword * ltype,CBuf * buf)
{
	...
	if(GetVersion()<0x80000000) rz=RegQueryValueExW(key,name,0,ltype,nullptr,&size);
	else rz=RegQueryValueExA(key,CStr(CWStr(name)).Get(),0,ltype,nullptr,&size);
