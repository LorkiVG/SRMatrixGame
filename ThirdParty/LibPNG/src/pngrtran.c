...
png_do_read_transformations(png_structp png_ptr)
{
   ...
   if (png_ptr->row_buf == 0)
   {
      ...
      sprintf(msg, "0 row buffer for row %ld, pass %d", png_ptr->row_number,
         png_ptr->pass);
      png_error(png_ptr, msg);
#else
      png_error(png_ptr, "0 row buffer");
#endif
   }
#endif
...
#if defined(PNG_USELESS_TESTS_SUPPORTED)
   if (row != 0 && row_info != 0 && row_info->bit_depth < 8)
#else
...
#if defined(PNG_USELESS_TESTS_SUPPORTED)
   if (row != 0 && row_info != 0 && row_info->bit_depth == 16)
#else
