...
png_write_flush(png_structp png_ptr)
{
   ...
   do
   {
      ...
      if(ret != Z_OK)
      {
         if(png_ptr->zstream.msg != 0) png_error(png_ptr, png_ptr->zstream.msg);
...
png_set_filter(png_structp png_ptr, int method, int filters)
{
   ...
   if (method == PNG_FILTER_TYPE_BASE)
   {
      ...
      {
         if (png_ptr->do_filter & PNG_FILTER_SUB && png_ptr->sub_row == 0)
         {
         ...
         if (png_ptr->do_filter & PNG_FILTER_UP && png_ptr->up_row == 0)
         {
         ...
         if (png_ptr->do_filter & PNG_FILTER_AVG && png_ptr->avg_row == 0)
         {
            if (png_ptr->prev_row == 0)
            {
